...
public class MarketplaceService : IMarketplaceService
{
    ...
    public MarketplaceService(FutsalContext db, ILogger<MarketplaceService> logger) { _db = db; _logger = logger; }
    ...
    public async Task RespondToPlayerRequestAsync(Guid userId, Guid requestId, RespondToRequestDto dto, CancellationToken ct = default)
    {
        ...
        // Invite: player responds. Application: owner responds.
        ...
        {
            throw new ConflictException("Player has already joined a team. The request could not be completed.");
        }
    }
    ...
    public async Task RespondToStaffRequestAsync(Guid userId, Guid requestId, RespondToRequestDto dto, CancellationToken ct = default)
    {
        ...
        {
            throw new ConflictException("Staff member has already joined a team. The request could not be completed.");
        }
    }

    public async Task CancelPlayerRequestAsync(Guid userId, Guid requestId, CancellationToken ct = default)
    {
        var request = await _db.PlayerTeamRequests
            .Include(r => r.Team)
            .Include(r => r.Player)
    ...
    public async Task CancelStaffRequestAsync(Guid userId, Guid requestId, CancellationToken ct = default)
    {
        var request = await _db.StaffTeamRequests
            .Include(r => r.Team)
            .Include(r => r.Staff)
    ...
    public async Task PlayerLeaveTeamAsync(Guid userId, CancellationToken ct = default)
    {
        var player = await _db.Players.FirstOrDefaultAsync(p => p.UserId == userId, ct)
            ?? throw new NotFoundException("Player profile not found.");

        var membership = await _db.TeamMembers
            .FirstOrDefaultAsync(m => m.PlayerId == player.PlayerId && m.LeftAt == null, ct)
            ?? throw new NotFoundException("You are not currently on a team.");
    ...
    public async Task StaffLeaveTeamAsync(Guid userId, CancellationToken ct = default)
    {
        var staff = await _db.Staff.FirstOrDefaultAsync(s => s.UserId == userId, ct)
            ?? throw new NotFoundException("Staff profile not found.");

        var assignment = await _db.TeamStaff
            .FirstOrDefaultAsync(s => s.StaffId == staff.StaffId && s.LeftAt == null, ct)
            ?? throw new NotFoundException("You are not currently assigned to a team.");
    ...
    public async Task<IEnumerable<PlayerRequestDto>> GetPlayerRequestsForTeamAsync(Guid ownerUserId, Guid teamId, CancellationToken ct = default)
    {
        ...
        return await _db.PlayerTeamRequests
            .AsNoTracking()
            .Include(r => r.Team)
            .Include(r => r.Player)
            .Where(r => r.TeamId == teamId)
            .OrderByDescending(r => r.CreatedAt)
            .Select(r => ToPlayerRequestDto(r))
            .ToListAsync(ct);
    }
    ...
    public async Task<IEnumerable<StaffRequestDto>> GetStaffRequestsForTeamAsync(Guid ownerUserId, Guid teamId, CancellationToken ct = default)
    {
        ...
        return await _db.StaffTeamRequests
            .AsNoTracking()
            .Include(r => r.Team)
            .Include(r => r.Staff)
            .Where(r => r.TeamId == teamId)
            .OrderByDescending(r => r.CreatedAt)
            .Select(r => ToStaffRequestDto(r))
            .ToListAsync(ct);
    }
