...
namespace CoreFutsal.Match.Controllers;
...
public class MatchesController : ControllerBase
{
    ...
    public async Task<IActionResult> GetByTeam(
        Guid teamId, [FromQuery] int page = 1, [FromQuery] int pageSize = 20, CancellationToken ct = default)
        => Ok(await _matches.GetMatchesForTeamAsync(teamId, page, pageSize, ct));

    [HttpGet("stadium/{stadiumId:guid}")]
    [AllowAnonymous]
    public async Task<IActionResult> GetByStadium(
        Guid stadiumId, [FromQuery] int page = 1, [FromQuery] int pageSize = 20, CancellationToken ct = default)
        => Ok(await _matches.GetMatchesForStadiumAsync(stadiumId, page, pageSize, ct));

    // ── Match requests ────────────────────────────────────────────────────────

    [HttpPost("requests")]
    [Authorize(Roles = "TeamOwner,Staff")]
